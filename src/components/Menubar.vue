...
<template>
	<div class="card">
		<Menubar style="border: 0" :model="items">
			<template #start>
			...
			<template #item="{ item, props, hasSubmenu, root }">
				<router-link v-slot="{ href, navigate }" :to="item.route" custom>
					<a v-ripple :href="href" class="flex align-items-center" v-bind="props.action">
			...
			<template #end>
				<div class="flex align-items-center gap-2">
					<InputText placeholder="搜尋" type="text" class="w-8rem sm:w-auto" />
					<Button type="button" label="登入/註冊" icon="pi pi-user" @click="showLogin" />
				</div>
			</template>
		</Menubar>
	</div>
